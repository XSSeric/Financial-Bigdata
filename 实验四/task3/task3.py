...
# 查准率
precision = predictions.filter(predictions.prediction == 1).filter(predictions.prediction == predictions.TARGET).count() / float(predictions.filter(predictions.prediction == 0).count())
print("Precision:", precision)
# 查全率
recall = predictions.filter(predictions.prediction == 1).filter(predictions.prediction == predictions.TARGET).count() / float(predictions.filter(predictions.TARGET == 0).count())
print("Recall:", recall)
# F1值
